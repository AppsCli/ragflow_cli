...
- `com.iotserv.RAGFlowCli.yml` -- Flatpak manifest, contains the Flatpak
...
```bash
flatpak-builder --force-clean build-dir com.iotserv.RAGFlowCli.yml --repo=repo
```
...
```bash
flatpak build-bundle repo RAGFlow-CLI.flatpak com.iotserv.RAGFlowCli
```
...
```bash
flatpak install --user RAGFlow-CLI.flatpak
```

### We can see that it is installed:

```bash
flatpak list --app | grep com.iotserv.RAGFlowCli
```

> RAGFlow CLI	com.iotserv.RAGFlowCli	1.0.0	master	ragflow-cli-origin	user

If we search for "RAGFlow CLI" in the system application menu there should be an
entry for the app with the proper name and icon.

We can also uninstall our test flatpak:

```bash
flatpak remove com.iotserv.RAGFlowCli
```
